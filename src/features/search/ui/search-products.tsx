  ...
  return (
    <div className={['relative flex-1 group', className].filter(Boolean).join(' ')}>
      <div className="relative z-20">
        {/* <SearchIcon className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-text-muted transition-colors group-focus-within:text-brand-400" /> */}
        <TextField
          name="поиск"
          type="text"
          value={value}
      ...
      {data && (
        <div className="absolute left-0 right-0 top-[calc(100%+8px)] z-10 overflow-hidden rounded-2xl border border-stroke-500 bg-ink-900/95 backdrop-blur-xl shadow-2xl animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          {/* <div className="border-t border-stroke-500 bg-white/[0.02] px-4 py-2 text-center">
            <button className="text-[11px] font-medium text-text-muted hover:text-brand-400 transition-colors">
